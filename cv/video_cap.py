...
def update_move_file(robot_targ_x, robot_targ_y):
	...
	move_file.close()
	print("Updating file with new move")
...
def clear_move_file():
	move_file = open("new_move.txt", "w")
	move_file.write("")
	move_file.close()

def check_move_busy():
	move_file = open("new_move.txt", "r") # open file for reading
	print("Checking if file busy with a move")
...
while (True):
	if (True):
		...
		hsvImage = cv2.cvtColor(min_frame, cv2.COLOR_BGR2HSV)
		...
		if bbox is not None:
			...
			cv2.rectangle(min_frame, (pixel_x1, pixel_y1), (pixel_x2, pixel_y2), (0,0,255), 5)
